...
class Word:
    ...
    def validadeNode(self):
      #print("Testes:")
      #print(self.parsing_word[-1:])
      if not self.word.strip():
        if not self.parsing_word[-1:].istitle():
          self.validate = True
    ...
    def getChildren(self):
        ...
        for i, (prod, regra) in enumerate(valid_rules):
            ...
            children.append(Word(new_word,prod_rule,self.rules, self.value + 1, self.parsing_word[:-1] + new_parsing_word + prod_rule, self))
        return children
